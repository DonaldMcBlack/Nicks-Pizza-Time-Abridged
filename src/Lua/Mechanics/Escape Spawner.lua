...
addHook("MobjThinker", function(spawner)
	...
	and (spawner.target and spawner.target.player.PTRound and spawner.target.player.PTRound.laps ~= spawner.spawnlap) then
		--Spawn in
		spawner.state = S_ESCAPESPAWNER1
		spawner.spawnlap = spawner.target.player.PTRound.laps
		spawner.tics = 1
